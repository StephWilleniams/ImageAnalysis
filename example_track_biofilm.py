# Bacterial colony identification code
...
path = "biofilm_plates/20231030151135.png" # Non-value optimised example (based on prev).
...
# Image processing.
img_GS_inv = 1-img_GS # Convert from brightfield to darkfield.
img_GS_inv = nd.gaussian_filter(img_GS_inv,0) # Optional Gaussian filter, currently off.
thresh = 0.6 # Threshold to convert the image to a binary.
...
hough_radii = np.arange(5, 25, 1)  # Circle radii to attempt to locate (min,max,incr.)
